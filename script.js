...
                <img data-src="${thumbnailSrc || 'images/thumbnails/default.jpg'}" src="${thumbnailSrc || 'images/thumbnails/default.jpg'}" alt="${match.title}" class="lazyload" width="350" height="200" loading="lazy">
            </picture>
...
            let lazyLoadImages = container.querySelectorAll('.lazyload');
            let imageObserver = new IntersectionObserver(function(entries, observer) {
                entries.forEach(function(entry) {
                    if (entry.isIntersecting) {
                        let image = entry.target;
                        // Only load if data-src exists and actual src is not already set
...
                            }
                        }
                        image.classList.remove('lazyload'); // Remove lazyload class once loaded
...
                imageObserver.observe(image); // Start observing each lazyload image
            });
        } else {
...
            history.pushState({ view: 'details', id: matchId }, match.title, newUrl.toString()); // Update browser history without full page reload
...
        setInterval(checkDevTools, 1000); // Periodically check every second
